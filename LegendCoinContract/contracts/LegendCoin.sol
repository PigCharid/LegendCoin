...
// File: LegendCoin1.sol

pragma solidity ^0.8.0;

contract LegendCoin is
    Context,
    AccessControlEnumerable,
    ERC20,
    ReentrancyGuard
{
    ...
    constructor() ERC20("LegendCoin", "CQBI") {
        _mint(community, 14 * 10**8 * 10**18);
        _mint(research, 26 * 10**8 * 10**18);
        _mint(airdrop, 24 * 10**8 * 10**18);
        _mint(promote, 16 * 10**8 * 10**18);
        _mint(mining, 20 * 10**8 * 10**18);
    ...
    function claimRewards() external nonReentrant {
        ...
        IERC20(address(this)).transferFrom(mining, _msgSender(), rewards);
    }

    function withdrawAll() external nonReentrant {
        require(deposited[_msgSender()] > 0, "You have no deposit");
        uint256 _rewards = calculateRewards(_msgSender()) +
            unclaimedRewards[msg.sender];
        uint256 _deposit = deposited[_msgSender()];
        deposited[_msgSender()] = 0;
        lastStakeTime[_msgSender()] = 0;
        unclaimedRewards[_msgSender()] = 0;
        ...
        AlreadyCliamStakeReward[_msgSender()] = totalClaimStakeReward;
        IERC20(address(this)).transferFrom(mining, _msgSender(), _amount);
    }
